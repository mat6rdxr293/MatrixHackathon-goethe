...
const localLlmService_1 = require("./llm/localLlmService");
...
const LOCAL_MENTOR_STEP_TIMEOUT_MS = Math.max(parsePositiveTimeout(process.env.LOCAL_LLM_MENTOR_TIMEOUT_MS, LOCAL_MENTOR_BASE_TIMEOUT_MS), 12000);
...
const formatList = (values, limit = 3) => values
    .filter(Boolean)
    .slice(0, limit)
    .join(", ");
const fallbackChatReply = (input) => {
    ...
    if (isDiscussQuestion) {
        const topics = [topRiskMessage, ...riskFlags, ...weaknesses, ...negativeTrendSubjects].filter((value) => Boolean(value));
        const discussionBase = topics.length > 0
        ...
        const extra = recommendationPool.length > 0
    ...
    if (isRiskQuestion) {
        if (input.role === "teacher" && (analytics?.teacherTopRisks?.length ?? 0) > 0) {
            return `По классу самый высокий риск у: ${formatList(analytics?.teacherTopRisks ?? [], 3)}. Первый шаг: ${recommendationPool[0] ??
        ...
        if (input.role === "admin" && (analytics?.adminTopRiskClasses?.length ?? 0) > 0) {
            return `По школе зоны внимания: ${formatList(analytics?.adminTopRiskClasses ?? [], 3)}. Первый шаг: ${recommendationPool[0] ??
    ...
    if (weaknesses.length > 0 || strengths.length > 0) {
        ...
        const actionText = recommendationPool.length > 0 ? `Следующий шаг: ${recommendationPool[0]}.` : "";
...
const requestResponsesText = async (config, payload) => {
    ...
    try {
        ...
        const response = await axios_1.default.post(RESPONSES_API_URL, { model: config.model, ...requestPayload }, {
            headers: {
...
const tryLocalMentorResponse = async (input) => {
    const localText = await localLlmService_1.localLlmService.generateText({
        systemPrompt: "You are a school AI analyst. Use only input data and do not invent facts. " +
...
const tryLocalClassReport = async (input) => {
    const localText = await localLlmService_1.localLlmService.generateText({
        systemPrompt: "You are a school assistant. Write a short practical class report in Russian. Plain text, no JSON.",
...
const tryLocalChatReply = async (input) => {
    const localText = await localLlmService_1.localLlmService.generateText({
        systemPrompt: "You are a school AI assistant. Reply briefly in Russian using only provided context. Do not invent facts.",
