...
exports.classReportService = {
    async buildClassReport(classId, profiles, teacherName) {
        ...
        const predictions = classProfiles
            .map((item) => (0, riskEngineService_1.studentPrediction)(item))
            .sort((a, b) => b.overallRisk - a.overallRisk);
        const summary = {
            students: classProfiles.length,
            averageScore: classProfiles.reduce((sum, item) => sum + item.averageScore, 0) / classProfiles.length,
        ...
        const reportText = await openAiMentorService_1.openAiMentorService.generateClassReport({
            teacherName,
            classId,
            summary: {
