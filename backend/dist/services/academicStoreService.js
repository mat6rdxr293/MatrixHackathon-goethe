...
const sanitizeJournalScoreRaw = (value) => value
    .replace(/РІР‚вЂќ/g, "-")
    .replace(/вЂ”/g, "-")
    .replace(/—/g, "-")
    .replace(/вЂ/g, "-")
...
const mapAchievement = (row) => ({
    ...
    proofAttachment: row.proof_file_name && row.proof_file_mime && row.proof_file_data_url
        ? {
...
const ensureOptions = (items, fallbackPrefix) => {
...
const normalizeAnswerText = (value) => normalizeText(value).toLowerCase();
const mapSubjectPracticeQuestion = (row) => {
    const base = {
        id: row.id,
        subject: row.subject,
        prompt: row.prompt,
    ...
    if (row.type === "single_choice") {
        const options = ensureOptions(parseJsonValue(row.options_json, []), "opt");
    ...
    if (row.type === "multiple_choice") {
        ...
        const parsedIds = ensureStringList(parseJsonValue(row.correct_option_ids_json, []));
    ...
    if (row.type === "short_answer") {
        return {
            ...base,
            type: "short_answer",
            acceptedAnswers: ensureStringList(parseJsonValue(row.accepted_answers_json, [])),
        };
    }
    if (row.type === "matching") {
        return {
            ...base,
            type: "matching",
            leftItems: ensureOptions(parseJsonValue(row.left_items_json, []), "left"),
            rightItems: ensureOptions(parseJsonValue(row.right_items_json, []), "right"),
            correctPairs: ensurePairs(parseJsonValue(row.correct_pairs_json, [])),
        };
    }
    return {
        ...base,
        type: "ordering",
        items: ensureOptions(parseJsonValue(row.order_items_json, []), "ord"),
        correctOrder: ensureStringList(parseJsonValue(row.correct_order_json, [])),
    };
};
...
const buildDefaultSubjectQuestions = (subject) => {
    ...
    const leftItems = seed.matchPairs.map((pair, index) => ({
        id: `left-${index + 1}`,
        text: pair.left,
    }));
    const rightItems = seed.matchPairs.map((pair, index) => ({
        id: `right-${index + 1}`,
        text: pair.right,
    }));
    const correctPairs = leftItems.map((left, index) => ({
        leftId: left.id,
        rightId: rightItems[index]?.id ?? "",
    }));
    const orderItems = seed.orderSteps.map((text, index) => ({
        id: `step-${index + 1}`,
        text,
    }));
    return [
...
const seedSubjectPracticeQuestions = () => {
    const total = db.prepare("SELECT COUNT(1) as total FROM subject_practice_questions").get();
    if (total.total > 0) {
        return;
    }
    const insert = db.prepare(`INSERT INTO subject_practice_questions (
    ...
    const tx = db.transaction(() => {
        ...
        for (const subject of Object.keys(DEFAULT_SUBJECT_SEEDS)) {
            ...
            for (const question of questions) {
                insert.run({
                    id: `spq-${(0, node_crypto_1.randomUUID)().slice(0, 12)}`,
                    subject,
                    type: question.type,
                    prompt: question.prompt.trim(),
                    explanation: question.explanation?.trim() || null,
                    options_json: question.type === "single_choice" || question.type === "multiple_choice"
                        ? JSON.stringify(question.options)
                        : null,
                    correct_option_id: question.type === "single_choice" ? question.correctOptionId : null,
                    correct_option_ids_json: question.type === "multiple_choice" ? JSON.stringify(question.correctOptionIds) : null,
                    accepted_answers_json: question.type === "short_answer" ? JSON.stringify(question.acceptedAnswers) : null,
...
const normalizeOptionsInput = (options, prefix) => {
    const result = [];
...
const normalizePairsInput = (pairs) => {
    const result = [];
...
const normalizeQuestionInput = (question) => {
    ...
    const sortOrder = typeof question.sortOrder === "number" && Number.isFinite(question.sortOrder)
    ...
    if (question.type === "matching") {
        ...
        const correctPairs = normalizePairsInput(question.correctPairs).filter((pair) => leftSet.has(pair.leftId) && rightSet.has(pair.rightId));
        return {
            type: "matching",
            prompt,
            explanation,
...
exports.academicStoreService = {
    ...
    `).run({
            ...
            verification_evidence: payload.proofAttachment?.fileName?.trim() ||
    ...
    listStudentJournalScopes(studentIdRaw) {
    ...
    replaceStudentJournalGrades(payload) {
        ...
        const deleteScoped = db.prepare(`DELETE FROM student_journal_grades
       WHERE student_id = @student_id
         AND edu_year = @edu_year
         AND period = @period
         AND period_type = @period_type`);
        const insertGrade = db.prepare(`INSERT INTO student_journal_grades (
        id, student_id, edu_year, period, period_type,
        subject_id, subject_uuid, subject_name,
        schedule_uuid, lesson_date, lesson_time, mark_type, mark_max,
        score_raw, score_five, synced_at
      ...
      VALUES (
        ...
        const tx = db.transaction((rows) => {
            ...
            for (const row of rows) {
                ...
                insertGrade.run({
                    id: row.id?.trim() || `jgr-${(0, node_crypto_1.randomUUID)().slice(0, 12)}`,
                    student_id: studentId,
                    edu_year: scope.eduYear,
                    period: scope.period,
                    period_type: scope.periodType,
                    subject_id: typeof row.subjectId === "number" && Number.isFinite(row.subjectId) ? Math.round(row.subjectId) : null,
                    ...
                    mark_max: typeof row.markMax === "number" && Number.isFinite(row.markMax) ? Number(row.markMax.toFixed(2)) : null,
                    score_raw: sanitizeJournalScoreRaw(row.scoreRaw),
                    score_five: typeof row.scoreFive === "number" && Number.isFinite(row.scoreFive)
    ...
    listSubjectPracticeQuestions(subjectRaw, includeAnswers = false) {
    ...
    createSubjectPracticeQuestion(payload) {
        ...
        const id = `spq-${(0, node_crypto_1.randomUUID)().slice(0, 12)}`;
        const now = new Date().toISOString();
        db.prepare(`INSERT INTO subject_practice_questions (
        id, subject, type, prompt, explanation,
        options_json, correct_option_id, correct_option_ids_json,
        accepted_answers_json,
        left_items_json, right_items_json, correct_pairs_json,
        ...
        @sort_order, @created_by, @created_at, @updated_at
            ...
            correct_option_ids_json: normalized.type === "multiple_choice" ? JSON.stringify(normalized.correctOptionIds) : null,
    ...
    updateSubjectPracticeQuestion(payload) {
        ...
        const now = new Date().toISOString();
        db.prepare(`UPDATE subject_practice_questions
       SET
         type = @type,
         prompt = @prompt,
       ...
       WHERE id = @id AND subject = @subject`).run({
            id: questionId,
            subject,
            type: normalized.type,
            prompt: normalized.prompt,
            explanation: normalized.explanation ?? null,
            options_json: normalized.type === "single_choice" || normalized.type === "multiple_choice"
    ...
    deleteSubjectPracticeQuestion(subjectRaw, questionIdRaw) {
        const subject = normalizeText(subjectRaw).toLowerCase();
    ...
    evaluateSubjectPracticeSubmission(payload) {
        ...
        const items = payload.answers.map((entry) => {
            ...
            if (question.type === "matching" && answer.type === "matching") {
                ...
                const submitted = new Map(answer.pairs
                    .map((pair) => ({
            ...
            const submittedOrder = answer.type === "ordering"
                ? answer.order.map((item) => normalizeText(item)).filter(Boolean)
                : [];
            const isCorrect = expectedOrder.length > 0 &&
    ...
    recordSubjectSession(payload) {
        ...
        const now = new Date().toISOString();
        db.prepare(`INSERT INTO subject_sessions (id, student_id, subject, task_id, score, time_spent_seconds, completed_at)
       VALUES (@id, @studentId, @subject, @taskId, @score, @timeSpentSeconds, @completedAt)`).run({
            id,
    ...
    listSubjectSessions(studentId) {
        const rows = db
            .prepare(`SELECT id, student_id, subject, task_id, score, time_spent_seconds, completed_at
    ...
    getSubjectStats(studentId) {
        const rows = db
            .prepare(`SELECT subject, COUNT(*) as sessions_count, AVG(score) as avg_score
