...
import { localizeSubjectName, SubjectLang } from "../utils/subjectNameLocalization";
...
type BilimClassConfig = {
  ...
  diaryWeeksPath: string;
  groupListPath: string;
  journalServiceUrl: string;
  journalServiceLegacyUrl: string;
  loginValue: string;
...
type StudentJournalResponse = {
  role: "student" | "parent";
  source: "bilimclass" | "cache" | "empty";
  studentId: string;
  selected: JournalFilterScope;
  filters: {
    years: number[];
    periods: number[];
    periodTypes: string[];
  ...
  subjects: JournalSubjectView[];
  grades: JournalGradeRow[];
};

type JournalAttestationItem = {
  subjectUuid: string | null;
  subjectName: string;
  mark: string | null;
  recommendedMark: string | null;
};

type DiaryFinalMarkItem = {
  subjectUuid: string | null;
  subjectName: string;
  finalMark: string;
};

type JournalScheduleMeta = {
  subjectName: string;
  subjectId: number | null;
  subjectUuid: string | null;
  scheduleUuid: string;
...
const authRuntimeByKey = new Map<string, BilimClassAuthRuntime>();
...
const extractHintsFromAuthUserInfo = (payload: unknown): BilimClassAuthHints => {
  const hints: BilimClassAuthHints = {
    schoolIds: [],
    groupIds: [],
    eduYears: [],
...
const extractAuthRuntime = (payload: unknown): BilimClassAuthRuntime => {
  const result: BilimClassAuthRuntime = {
    chatToken: null,
    userId: null,
  ...
  const userInfo = isRecord(payload.user_info)
    ...
    : nestedData && isRecord(nestedData.user_info)
      ...
      : null;
  const studentInfo =
    userInfo && isRecord(userInfo.studentInfo)
      ? userInfo.studentInfo
      : userInfo && isRecord(userInfo.student_info)
        ? userInfo.student_info
        : null;

  const chatTokenCandidates: unknown[] = [
  ...
  const pushGroupUuid = (candidate: unknown) => {
...
const withDefaultBilimHeaders = (headers: Record<string, string>): Record<string, string> => ({
...
const extractAttestationItems = (payload: unknown): JournalAttestationItem[] => {
  ...
  const result: JournalAttestationItem[] = [];
...
const extractFinalMarksFromDiarySubjects = (subjects: Record<string, unknown>[]): DiaryFinalMarkItem[] => {
  const result: DiaryFinalMarkItem[] = [];
...
const calculateCurrentAndTrendFromHistory = (history: GradePoint[]) => {
  if (history.length === 0) {
...
const buildProfileFromJournalGrades = (
  binding: RuntimeStudentBinding,
  grades: JournalGradeRow[],
  fallbackProfile: StudentProfile | null,
): StudentProfile | null => {
  ...
  for (const grade of grades) {
    ...
    const markMaxHint =
      typeof grade.markMax === "number" && Number.isFinite(grade.markMax) ? grade.markMax : null;
    const parsedFromRaw = parseScoreCandidate(grade.scoreRaw, markMaxHint);
    const parsedScore =
      parsedFromRaw !== null && Number.isFinite(parsedFromRaw)
        ? parsedFromRaw
...
const enrichProfilesFromJournalCache = (profiles: StudentProfile[], config: BilimClassConfig): StudentProfile[] => {
  const enriched = new Map<string, StudentProfile>();

  for (const profile of profiles) {
    const scopes = academicStoreService
      .listStudentJournalScopes(profile.studentId)
      .sort((a, b) => b.lastSyncedAt.localeCompare(a.lastSyncedAt));
    let cachedGrades: JournalGradeRow[] = [];
    for (const scope of scopes) {
      cachedGrades = academicStoreService.listStudentJournalGrades({
    ...
    const binding: RuntimeStudentBinding = {
      studentId: profile.studentId,
      ...
      schoolId:
        pickMappedNumber(config.schoolByStudent, profile.studentId) ??
...
const resolveStudentIdForJournal = (user: Pick<User, "id" | "role" | "linkedStudentId">): string | null => {
...
const normalizePeriodType = (value: unknown, fallback = "quarter") => {
...
const resolveJournalScope = (payload: {
  requested?: Partial<JournalFilterScope>;
  binding: RuntimeStudentBinding;
  config: BilimClassConfig;
  scopes: JournalScopeInfo[];
}): JournalFilterScope => {
  ...
  const selectedEduYear =
    typeof requested?.eduYear === "number" && Number.isFinite(requested.eduYear)
  ...
  const selectedPeriod =
    typeof requested?.period === "number" && Number.isFinite(requested.period)
...
const pickRawScoreValue = (value: unknown, depth = 0): string => {
...
const normalizeLessonTime = (value: unknown) => {
  const raw = cleanString(value);
  if (!raw) {
    return "";
  }
...
const normalizeJournalMarkType = (value: unknown) => {
  const normalized = cleanString(value).toLowerCase();
  if (!normalized) {
    return "regular";
  }
...
const extractJournalGradesFromDiary = (payload: {
  ...
  for (const [subjectIndex, subject] of payload.subjects.entries()) {
    ...
    for (const [scheduleIndex, rawSchedule] of schedules.entries()) {
      ...
      const rawMarkType =
        pickString(rawSchedule, ["type", "markType"]) ||
        pickString(detail ?? {}, ["markType"]) ||
        pickString(
          isRecord(detail?.formattedScore) ? (detail.formattedScore as Record<string, unknown>) : {},
          ["markType"],
        );
      const markType = normalizeJournalMarkType(rawMarkType);
      const markMax =
        pickNumber(rawSchedule, ["markMax", "maxMark", "max_score", "max"]) ??
      ...
      const rawPoints =
        pickDeepNumberByKeys(detail, ["mark", "score", "value", "result", "grade"]) ??
...
const buildJournalSubjectsView = (
  grades: JournalGradeRow[],
  finalMarkByUuid: Map<string, string>,
  finalMarkByName: Map<string, string>,
): JournalSubjectView[] => {
  const bySubject = new Map<
    string,
    {
  ...
  return [...bySubject.values()]
    .map((item) => ({
      ...
      finalMark:
        (item.subjectUuid ? finalMarkByUuid.get(item.subjectUuid) : undefined) ??
...
const resolveJournalBindingForUser = (
  user: Pick<User, "id" | "role" | "name" | "classId" | "linkedStudentId">,
  config: BilimClassConfig,
  profiles: StudentProfile[],
): { studentId: string; role: "student" | "parent"; binding: RuntimeStudentBinding; credentials: BilimCredentials | null } | null => {
  ...
  const userBinding = storageService.getBilimBinding(user.id);
  const linkedAccounts = storageService.listBilimLinkedUsers();
  ...
  let credentials: BilimCredentials | null = null;
  if (userBinding?.linked && userBinding.login) {
    ...
    const studentLinkedAccount = linkedAccounts.find(
      (item) => item.role === "student" && item.userId === studentId,
    );
...
const getConfig = (): BilimClassConfig => {
  ...
  const diaryWeeksPath = normalizePath(
    process.env.BILIMCLASS_DIARY_WEEKS_PATH ?? "/api/v4/os/clientoffice/diary/weeks",
  );
  const groupListPath = normalizePath(process.env.BILIMCLASS_GROUP_LIST_PATH ?? "/api/v4/os/clientoffice/group-list");
  const journalServiceUrl = trimSlashes(
    process.env.BILIMCLASS_JOURNAL_SERVICE_URL?.trim() ?? "https://journal-service.bilimclass.kz",
  );
  const journalServiceLegacyUrl = trimSlashes(
    process.env.BILIMCLASS_JOURNAL_SERVICE_LEGACY_URL?.trim() ?? journalServiceUrl,
  );
...
const extractGroupIds = (payload: unknown): number[] => {
  const result: number[] = [];
  const push = (value: unknown) => pushUniqueNumber(result, value, (candidate) => candidate > 1000);
...
const fetchGroupIds = async (
  ...
  try {
    const response = await axios.get(`${config.baseUrl}${config.groupListPath}`, {
...
const resolveJournalServiceBaseUrl = (config: BilimClassConfig, eduYear: number) => {
...
const extractDiaryWeekDates = (payload: unknown): string[] => {
  const result: string[] = [];
  const pushDate = (candidate: unknown) => {
...
const fetchDiaryWeekDates = async (payload: {
  ...
  try {
    const response = await axios.get(`${config.baseUrl}${config.diaryWeeksPath}`, {
      timeout: config.timeoutMs,
      params: {
        schoolId: binding.schoolId,
        eduYear: scope.eduYear,
...
const extractScheduleMetaFromDiarySubjects = (subjects: Record<string, unknown>[]) => {
  const byScheduleUuid = new Map<string, JournalScheduleMeta[]>();
  const dedupe = new Set<string>();
...
const extractJournalServiceMap = (payload: unknown) => {
  const byScheduleUuid = new Map<string, Record<string, unknown>>();
...
const calculateJournalDetailStrength = (detail: Record<string, unknown>) => {
  let strength = 0;
...
const fetchJournalServiceScoresForScope = async (payload: {
  config: BilimClassConfig;
  auth: BilimClassAuthResponse;
  authRuntime: BilimClassAuthRuntime;
  binding: RuntimeStudentBinding;
  scope: JournalFilterScope;
...
const pickJournalMarkRecordByType = (detail: Record<string, unknown> | undefined, markType: string) => {
  if (!detail) {
    return null;
  }

  const formatted = isRecord(detail.formattedScore) ? (detail.formattedScore as Record<string, unknown>) : null;
  const sor = isRecord(detail.sor) ? (detail.sor as Record<string, unknown>) : null;
  const soch = isRecord(detail.soch) ? (detail.soch as Record<string, unknown>) : null;
...
const formatMarkNumber = (value: number) => {
  if (!Number.isFinite(value)) {
    return "";
  }
...
const buildJournalGradesFromLiveSources = (payload: {
  ...
  scope: JournalFilterScope;
  syncedAt: string;
  scheduleMetaByUuid: Map<string, JournalScheduleMeta[]>;
  journalDetailsByUuid: Map<string, Record<string, unknown>>;
}) => {
  const result: JournalGradeRow[] = [];
  const dedupe = new Set<string>();
  ...
  for (const [scheduleUuid, metas] of payload.scheduleMetaByUuid.entries()) {
    ...
    for (const [metaIndex, meta] of metas.entries()) {
      ...
      const markMax =
        (markRecord ? pickNumber(markRecord, ["markMax", "maxMark", "max_score", "max"]) : null) ??
        meta.markMax;
      const markValue =
        (markRecord ? pickDeepNumberByKeys(markRecord, ["mark", "score", "value", "result", "grade"]) : null) ??
      ...
      const scoreFive = parseScoreCandidate(
        markRecord ?? detail ?? scoreRaw,
        markMax !== null && Number.isFinite(markMax) ? markMax : null,
      );
...
const fetchAttestationMarks = async (
  config: BilimClassConfig,
  auth: BilimClassAuthResponse,
  binding: RuntimeStudentBinding,
): Promise<JournalAttestationItem[] | null> => {
  if (!binding.groupId || !binding.schoolId || !binding.eduYear || !binding.period || !binding.periodType) {
    return null;
  }

  try {
    const response = await axios.get(`${config.baseUrl}/api/v4/os/clientoffice/attestation`, {
      timeout: config.timeoutMs,
      params: {
        schoolId: binding.schoolId,
        eduYear: binding.eduYear,
...
const syncProfilesForBindings = async (
  ...
  const groupListCache = new Map<string, Promise<number[]>>();
  ...
  const getGroupIdsForSchool = (authContextKey: string, auth: BilimClassAuthResponse, schoolId: number, eduYear: number) => {
  ...
  for (const item of bindings) {
    ...
    const discoverySchoolIds: number[] = [];
    ...
    const discoveryEduYears: number[] = [];
    ...
    const scope: JournalFilterScope = {
    ...
    const authRuntime =
      authRuntimeByKey.get(authContext.cacheKey) ??
      ({
        chatToken: null,
        userId: null,
        studentGroupUuid: null,
        studentGroupUuids: [],
      } as BilimClassAuthRuntime);
    ...
    const diaryDerivedGrades =
      liveJournalGrades.length > 0
        ? liveJournalGrades
    ...
    if (diaryDerivedGrades.length > 0) {
      academicStoreService.replaceStudentJournalGrades({
        studentId: item.binding.studentId,
        scope,
...
const collectUniqueSubjectsFromProfiles = (profiles: StudentProfile[], classId?: string) => {
...
const collectSubjectNamesFromDiaryPayload = (subjectsPayload: Record<string, unknown>[]) => {
  const unique = new Map<string, string>();
...
const fetchLiveJournalGrades = async (payload: {
  config: BilimClassConfig;
  binding: RuntimeStudentBinding;
  credentials: BilimCredentials;
  scope: JournalFilterScope;
  syncedAt: string;
}): Promise<
  | {
      grades: JournalGradeRow[];
      resolvedBinding: RuntimeStudentBinding;
      diaryFinalMarks: DiaryFinalMarkItem[];
    }
  | null
> => {
  ...
  const schoolDiscoveryCandidates: number[] = [];
  ...
  const bindingSeed: RuntimeStudentBinding = {
    ...payload.binding,
    eduYear: payload.scope.eduYear,
    period: payload.scope.period,
    periodType: payload.scope.periodType,
  ...
  const candidates = buildBindingCandidates(bindingSeed, payload.config, hints)
    .filter(
      (item) =>
        item.eduYear === payload.scope.eduYear &&
...
export const bilimClassService = {
  ...
  async getStudentJournal(
    user: Pick<User, "id" | "role" | "name" | "classId" | "linkedStudentId">,
    requestedScope?: Partial<JournalFilterScope>,
    requestedLang: SubjectLang = "ru",
  ): Promise<StudentJournalResponse> {
    if (user.role !== "student" && user.role !== "parent") {
      throw new Error("Journal is available only for student and parent roles");
    }

    const config = getConfig();
    const profiles = academicStoreService.listStudentProfiles();
    const context = resolveJournalBindingForUser(user, config, profiles);

    const fallbackBinding: RuntimeStudentBinding = context?.binding ?? {
    ...
    const scopeList = fallbackBinding.studentId
      ? academicStoreService.listStudentJournalScopes(fallbackBinding.studentId)
      : [];
    ...
    let source: StudentJournalResponse["source"] = "empty";
    let grades: JournalGradeRow[] = [];
    const finalMarkByUuid = new Map<string, string>();
    const finalMarkByName = new Map<string, string>();
    ...
    if (grades.length === 0 && fallbackBinding.studentId) {
      grades = academicStoreService.listStudentJournalGrades({
    ...
    const subjects = buildJournalSubjectsView(grades, finalMarkByUuid, finalMarkByName).map((item) => ({
      ...item,
      subjectName: localizeSubjectName(item.subjectName, requestedLang),
    }));
    const localizedGrades = grades.map((item) => ({
      ...item,
      subjectName: localizeSubjectName(item.subjectName, requestedLang),
    }));
    const selectedScopeMeta = updatedScopes.find(
      (item) =>
        item.eduYear === selected.eduYear &&
    ...
    return {
      ...
      stats: {
        ...
        lastSyncAt:
          selectedScopeMeta?.lastSyncedAt ??
          localizedGrades.reduce<string | null>((acc, item) => {
  ...
  async importScheduleSubjects(classId?: string): Promise<{ source: "bilimclass" | "database"; subjects: string[] }> {
    const config = getConfig();
    ...
    const linkedBindings = buildBindingsFromLinkedAccounts(fallbackProfiles, config).filter((item) =>
      targetClassId ? normalizeClassId(item.binding.classId) === targetClassId : true,
    );
    ...
    const subjectsCache = new Map<string, Promise<Record<string, unknown>[] | null>>();
    const groupListCache = new Map<string, Promise<number[]>>();
    const importedSubjects = new Map<string, string>();
    ...
    for (const item of candidateBindings) {
      ...
      const getSubjectsForBinding = (binding: RuntimeStudentBinding) => {
      ...
      const getGroupIdsBySchoolYear = (schoolId: number, eduYear: number) => {
      ...
      const yearDiscoveryCandidates: number[] = [];
  ...
  async getStudentProfiles(): Promise<StudentProfile[]> {
    ...
    if (config.useLive) {
      ...
      if (liveProfiles && liveProfiles.length > 0) {
        ...
        lastError = null;
        return enrichProfilesFromJournalCache(academicStoreService.listStudentProfiles(), config);
      }
    }
