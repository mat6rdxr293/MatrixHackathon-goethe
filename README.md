...
Мы реверсинженерингом сайта BilimClass, создали рабочий парсер оценок с платформы BilimClass, что позволит получать реальные оценки в реальном времени. 
- https://api.bilimclass.kz - это главный URL api bilimclass
- /api/v2/os/login - эндопоинт логина на платформе
- /api/v4/os/clientoffice/diary/subjects - эндроинт для получения оценок и предметов
-
  **ВНИМАНИЕ. Данные об оценках получаются только при авторизации под своим аккаунтом, мы не пытались, и не имели целей взломать или заполучить чужие данные(получаются данные только в пределах прав доступа вашего аккаунта). Никакие авторизационные данные не отправляются на сторонние сервера, и все делается исключительно на добровольной основе(не обязательный сервис)**
